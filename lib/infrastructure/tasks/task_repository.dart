...
class TaskRepository {
  ...
  Stream<List<Task>> fetchTasks() async* {
    yield* (_prefs.getStringListStream(tasksKey)).map((jsonTasks) =>
        (jsonTasks ?? [])
  ...
  Stream<List<Task>> fetchFilteredTasks(Status status) async* {
    yield* fetchTasks().map((tasks) => tasks
        .map((task) => task)
  ...
  Future<void> create(Task task) async {
    ...
    await _prefs.setStringList(
        tasksKey, [...?currentTasksJson, jsonEncode(task.toJson())]);
  }
...
List<Task> _convertToTasks(List<String>? listJson) => (listJson ?? [])
    .map((taskData) => Task.fromJson(jsonDecode(taskData)))
    .toList();
